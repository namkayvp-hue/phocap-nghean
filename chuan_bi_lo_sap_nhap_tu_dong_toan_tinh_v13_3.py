# -*- coding: utf-8 -*-
r"""
V13.3 - CHUẨN BỊ LÔ SÁP NHẬP TỰ ĐỘNG TOÀN TỈNH
================================================
...
def main() -> int:
    print("=" * 126)
    print("V13.3 - CHUẨN BỊ LÔ SÁP NHẬP TỰ ĐỘNG TOÀN TỈNH")
    print("=" * 126)
    ...
    out_dir = ROOT / f"bao_cao_v13_3_chuan_bi_lo_{stamp}"
    zip_path = ROOT / f"bao_cao_v13_3_chuan_bi_lo_{stamp}.zip"
    ...
    with db_connect(True) as con:
        ...
        active_years = [x for x in years if int(x.get("is_active") or 0) == 1]
        if len(active_years) != 1:
            raise RuntimeError(
                f"Phải có đúng 1 năm học active; hiện={len(active_years)}"
            )
        active_year = active_years[0]
        year_id = int(active_year["id"])
        year_code = clean(active_year.get("code") or active_year.get("name"))
    ...
    write_csv(
        out_dir / "717_GATE_V13_3.csv",
        ["check","result","detail"],
        gate,
    )

    summary = f"""V13.3 - CHUẨN BỊ LÔ SÁP NHẬP TỰ ĐỘNG TOÀN TỈNH
======================================================================
...
"""
    (out_dir / "00_TONG_QUAN_V13_3.txt").write_text(
        summary,
        encoding="utf-8",
    )
