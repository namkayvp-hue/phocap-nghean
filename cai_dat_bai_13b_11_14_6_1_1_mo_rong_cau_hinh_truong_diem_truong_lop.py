...
BACKUP = (
    PROJECT
    / "exports"
    / f"backup_bai_13b_11_14_6_1_1_{STAMP}"
)
...
def patch_main(
    ...
    if INCLUDE_MARKER not in patched:
        # Bài 13B-11.14.6.1.1:
        # Source thật có import staff_management_router nhưng đoạn
        # app.include_router(staff_management_router) không nằm tại
        # vị trí mà V14.6.1 giả định. Dùng một anchor include đã được
        # khảo sát chắc chắn có trong main.py; nếu cần thì fallback
        # sang include cuối cùng của file.
        include_candidates = (
            "app.include_router(staff_management_router)",
            "app.include_router(report_inputs_router)",
        ...
        if anchor is not None:
            ...
            patched = (
                ...
                + INCLUDE_MARKER
                + "\n"
                + "app.include_router(class_structure_inputs_router)"
                + patched[insert_at:]
            )
        else:
            matches = list(
                re.finditer(
                    r"(?m)^\s*app\.include_router\([^\n]+\)\s*$",
                    patched,
                )
            )
            ...
            if not matches:
                raise RuntimeError(
                    "Không tìm thấy bất kỳ app.include_router(...) "
                    "nào trong app/main.py để gắn router 14.6.1.1."
                )
...
def main() -> int:
    ...
    try:
        ...
        print("=" * 126)
        print(
            "CÀI ĐẶT BÀI 13B-11.14.6.1.1 THÀNH CÔNG"
        )
        print("=" * 126)
