...
NEW_MARKER = "BAI_13B_12_V2_3_8_1_SAVE_TWO_SESSIONS_TO_REAL_RECORD_START"

STAMP = datetime.now().strftime("%Y%m%d_%H%M%S")
BACKUP = EXPORTS / f"backup_bai_13b_12_v2_3_8_1_{STAMP}"
DB_BACKUP = BACKUP / "phocap.db"
REPORT = EXPORTS / f"bao_cao_cai_bai_13b_12_v2_3_8_1_{STAMP}.txt"
...
def patch_source(source: str) -> tuple[str, str]:
    if NEW_MARKER in source:
        return source, "V2.3.8.1 đã có; không cài lặp."

    tree = ast.parse(source)
    fn = find_function(tree)

    fn_lines = source.splitlines()
    fn_text = "\n".join(
        fn_lines[
            int(fn.lineno) - 1:
    ...
    if OLD_MARKER not in fn_text:
        raise RuntimeError(
            "Không thấy nền V2.3.7 trong đúng route lưu năm học."
        )

    # Sau rollback V2.3.8, không được còn phép gán trực tiếp vào record thực.
    existing_real = re.findall(
        r"(?m)^\s*record\.attends_two_sessions_per_day\s*=",
        fn_text,
    )
    if existing_real:
        raise RuntimeError(
            "Source đã có phép gán vào record thực; "
            "không cài chồng V2.3.8.1."
        )
    ...
    if assign_node is None:
        raise RuntimeError(
            "Không tìm thấy 'record = existing_record'."
        )
    ...
    if if_node.lineno - assign_node.lineno > 8:
        raise RuntimeError(
            "Cấu trúc route khác bản khảo sát; dừng an toàn."
        )
    ...
    return (
        patched,
        "Đã gán Học 2 buổi/ngày vào đúng biến record thực tế.",
    )
...
def verify() -> None:
    ...
    if NEW_MARKER not in fn_text:
        raise RuntimeError("Thiếu marker V2.3.8.1 trong route.")

    # QUAN TRỌNG: regex neo đầu dòng để KHÔNG tính nhầm "form_record..."
    real_assignments = re.findall(
        r'(?m)^\s*record\.attends_two_sessions_per_day\s*=\s*'
        r'parsed_tri_state\["attends_two_sessions_per_day"\]\s*$',
        fn_text,
    )

    if len(real_assignments) != 1:
        raise RuntimeError(
            "Số phép gán TRỰC TIẾP vào record.attends_two_sessions_per_day "
            f"bất thường: {len(real_assignments)}"
        )

    # V2.3.7 có thể vẫn còn phép gán vào form_record; không coi đó là lỗi.
    form_record_assignments = re.findall(
        r'(?m)^\s*form_record\.attends_two_sessions_per_day\s*=',
        fn_text,
    )

    print(
        "Verifier: record thực =",
        len(real_assignments),
        "| form_record cũ =",
        len(form_record_assignments),
    )
...
def main() -> int:
    ...
    print(
        "BÀI 13B-12 V2.3.8.1 - "
        "SỬA VERIFIER + GHI HỌC 2 BUỔI/NGÀY VÀO ĐÚNG YEAR-RECORD"
    )
    print("=" * 122)
    ...
    REPORT.write_text(
        "\n".join(
            [
                "=" * 122,
                "BÁO CÁO CÀI BÀI 13B-12 V2.3.8.1",
                "=" * 122,
                f"Thời gian: {datetime.now():%d/%m/%Y %H:%M:%S}",
                "",
                "LỖI CỦA BỘ CÀI V2.3.8:",
                "- Verifier dùng chuỗi con 'record.attends...' nên đếm nhầm "
                  "cả 'form_record.attends...'.",
                "- Vì vậy thấy 2 và rollback dù source sửa đúng hướng.",
                "",
                "V2.3.8.1:",
                "- Verifier neo đầu dòng, phân biệt chính xác record và form_record.",
                f"- {action}",
                "- Không tự thay dữ liệu điều tra.",
                "",
    ...
    print()
    print("=" * 122)
    print("CÀI ĐẶT THÀNH CÔNG BÀI 13B-12 V2.3.8.1")
    print("=" * 122)
    print(" - Verifier đã sửa, không còn đếm nhầm form_record.")
    print(" - Học 2 buổi/ngày được gán vào đúng year-record.")
