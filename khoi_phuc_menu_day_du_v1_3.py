...
def main() -> int:
    ...
    if not latest.exists():
        print("KHONG TIM THAY THONG TIN BAN SAO V1.3.")
        return 1

    backup = Path(latest.read_text(encoding="utf-8").strip())
    manifest_path = backup / "MENU_V1_3_MANIFEST.json"
    ...
    print("\nKHOI PHUC MENU VE TRANG THAI TRUOC V1.3")
    ...
    elif PARTIAL_REL.as_posix() in set(manifest.get("created_files", [])):
        if partial_path.exists():
            partial_path.unlink()

    print("DA KHOI PHUC GIAO DIEN TRUOC V1.3.")
    print("KHONG KHOI PHUC / GHI DE CO SO DU LIEU.")
    print("KHONG THAY DOI app/main.py.")
    return 0
