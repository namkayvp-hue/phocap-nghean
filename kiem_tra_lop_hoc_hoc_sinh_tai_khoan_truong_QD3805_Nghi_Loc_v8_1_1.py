# -*- coding: utf-8 -*-
r"""
DRY-RUN V8.1.1 - KIỂM TRA LỚP HỌC + HỌC SINH TẠI TÀI KHOẢN NHÀ TRƯỜNG
QĐ 3805 - NGHI LỘC
====================================================================
...
----
    cd C:\PhoCap
    .\.venv\Scripts\python.exe .\kiem_tra_lop_hoc_hoc_sinh_tai_khoan_truong_QD3805_Nghi_Loc_v8_1_1.py

KẾT QUẢ
-------
    C:\PhoCap\dry_run_lop_hoc_hoc_sinh_tai_khoan_truong_QD3805_Nghi_Loc_v8_1_1_<timestamp>.zip
"""
...
def unique_indexes(conn: sqlite3.Connection, table: str) -> list[dict]:
    ...
    for r in conn.execute(f"PRAGMA index_list({qident(table)})").fetchall():
        ...
        # SQLite có thể trả None cho index dạng biểu thức.
        # Chuyển an toàn sang chuỗi để DRY-RUN không bị dừng.
        idx_cols = []
        for x in idx_rows:
            col_name = x[2]
            if col_name is None:
                idx_cols.append("<EXPRESSION>")
            else:
                idx_cols.append(str(col_name))
...
def audit() -> None:
    ...
    out_dir = PROJECT_ROOT / (
        f"dry_run_lop_hoc_hoc_sinh_tai_khoan_truong_QD3805_Nghi_Loc_v8_1_1_{ts}"
    )
    ...
    try:
        ...
        with summary.open("w", encoding="utf-8") as f:
            ...
            f.write("- V8.1.1 KHÔNG thay đổi database.\n")

        zip_path = PROJECT_ROOT / (
            f"dry_run_lop_hoc_hoc_sinh_tai_khoan_truong_QD3805_Nghi_Loc_v8_1_1_{ts}.zip"
        )
        ...
        print()
        print("=" * 114)
        print("HOÀN THÀNH DRY-RUN V8.1.1")
        print("=" * 114)
...
def main() -> None:
    try:
        ...
        print()
        print("=" * 114)
        print("LỖI KHÔNG DỰ KIẾN TRONG V8.1.1")
        print("=" * 114)
        print(repr(e))
        print("Database KHÔNG bị thay đổi.")
        sys.exit(4)
