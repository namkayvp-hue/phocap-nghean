...
BACKUP = EXPORTS / f"backup_bai_13b_12_v2_4_3_9_{STAMP}"
DB_BACKUP = BACKUP / "phocap.db"
REPORT = EXPORTS / f"bao_cao_cai_bai_13b_12_v2_4_3_9_{STAMP}.txt"
...
def replace_can_edit_in_function(
    ...
    matches = [i for i, line in enumerate(lines) if '"can_edit":' in line]
    if len(matches) != 1:
        raise RuntimeError(
            f"{function_name}: cần đúng 1 dòng can_edit, thực tế={len(matches)}"
        )
...
def patch_source(source: str) -> str:
    ...
    source = replace_can_edit_in_function(
        source,
        "csvc_input_page",
        '"can_edit": (scope["role_code"] in CSVC_EDIT_ROLES '
    ...
    source = replace_can_edit_in_function(
        ...
        '"can_edit": (scope["role_code"] in STRUCTURED_FORM_EDIT_ROLES '
        'or _v2439_is_school_unit(scope.get("user"), scope.get("role_code"))),',
        STRUCT_PAGE_MARKER,
    )
...
def main() -> int:
    ...
    lines = [
        "=" * 128,
        "BÁO CÁO CÀI BÀI 13B-12 V2.4.3.9",
        "=" * 128,
    ...
    print()
    print("=" * 128)
    print("CÀI ĐẶT THÀNH CÔNG BÀI 13B-12 V2.4.3.9")
    print("=" * 128)
