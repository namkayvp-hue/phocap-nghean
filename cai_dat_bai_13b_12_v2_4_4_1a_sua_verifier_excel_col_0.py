...
BACKUP = EXPORTS / f"backup_bai_13b_12_v2_4_4_1a_{STAMP}"
DB_BACKUP = BACKUP / "phocap.db"
REPORT = EXPORTS / f"bao_cao_cai_bai_13b_12_v2_4_4_1a_{STAMP}.txt"
...
def verify_catalog(source: str) -> None:
    ...
    for slug, (_title, fields) in CHECKLIST_BY_SLUG.items():
        ...
        for code, _label, _help in fields:
            ...
            excel_col = field.get("excel_col")
            if excel_col is None or int(excel_col) != 0:
                raise RuntimeError(
...
def main() -> int:
    ...
    report_lines = [
        "=" * 136,
        "BÁO CÁO CÀI BÀI 13B-12 V2.4.4.1A",
        "=" * 136,
        (
            "Thời gian: "
