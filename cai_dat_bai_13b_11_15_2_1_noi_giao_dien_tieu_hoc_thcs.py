...
BACKUP = (
    PROJECT
    / "exports"
    / f"backup_bai_13b_11_15_2_1_{STAMP}"
)
...
def find_year_form_bounds(
    ...
    if not close_match:
        raise RuntimeError(
            "Không tìm thấy </form> của year_record_form."
        )
...
def find_submit_anchor_in_year_form(
    source: str,
    form_start: int,
    form_close_start: int,
) -> int:
    form_text = source[
        form_start:
        form_close_start
    ...
    for pattern in patterns:
        ...
        if matches:
            # Ưu tiên nút submit cuối cùng trong đúng form,
            # vì template có thể có nút phụ phía trên.
            return (
                form_start
                + matches[-1].start()
            )

    # Không có nút submit nhận diện được:
    # chèn ngay trước </form> vẫn bảo đảm block nằm đúng form.
    return form_close_start
...
def patch_template(
    ...
    (
        ...
        form_close_start,
        _form_close_end,
    ) = find_year_form_bounds(
        source
    )

    submit_pos = find_submit_anchor_in_year_form(
        source,
        form_start,
        form_close_start,
    )
    ...
    if not (
        ...
        raise RuntimeError(
            "Vị trí chèn dữ liệu không nằm trong year_record_form."
        )
...
def main() -> int:
    ...
    print()
    print("SỬA SO VỚI V15.2:")
    print(
        " - Chỉ tìm nút Lưu bên trong đúng year_record_form."
    )
    print(
        " - Không còn nhầm nút submit của form khác phía trên template."
    )
    print(
        " - Nếu không thấy nút submit, chèn ngay trước </form> của year_record_form."
    )
    print()
    print("AN TOÀN:")
    print(
    ...
    try:
        ...
        print("=" * 132)
        print(
            "CÀI ĐẶT BÀI 13B-11.15.2.1 THÀNH CÔNG"
        )
        print("=" * 132)
