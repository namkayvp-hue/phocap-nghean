...
BACKUP = EXPORTS / f"backup_bai_13b_12_v2_4_3_4_{STAMP}"
DB_BACKUP = BACKUP / "phocap.db"
REPORT = EXPORTS / f"bao_cao_cai_bai_13b_12_v2_4_3_4_{STAMP}.txt"
...
def ensure_redirect_import(source: str) -> str:
    ...
    if "RedirectResponse" in first_part:
        return source

    if "from fastapi.responses import " in source:
        lines = source.splitlines(keepends=True)
    ...
    for i, line in enumerate(lines):
        if line.startswith("from __future__ import "):
            insert_at = i + 1
            break
    lines.insert(insert_at, "from fastapi.responses import RedirectResponse\n")
    result = "".join(lines)
    ast.parse(result)
    return result
...
def main() -> int:
    print("=" * 126)
    print("BÀI 13B-12 V2.4.3.4 - GOM 7 BIỂU MẦM NON VỀ MỘT PIPELINE MASTER")
    print("=" * 126)
    ...
    REPORT.write_text(
        "\n".join(
            [
                "=" * 126,
                "BÁO CÁO CÀI BÀI 13B-12 V2.4.3.4",
                "=" * 126,
    ...
    print()
    print("=" * 126)
    print("CÀI ĐẶT THÀNH CÔNG BÀI 13B-12 V2.4.3.4")
    print("=" * 126)
