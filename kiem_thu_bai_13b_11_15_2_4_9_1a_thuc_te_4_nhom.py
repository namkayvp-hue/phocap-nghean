...
KIỂM THỬ THỰC TẾ TỰ ĐỘNG KẾT LUẬN ĐẠT CHUẨN / MỨC ĐỘ
...
OUT_DIR = EXPORTS / f"kiem_thu_bai_13b_11_15_2_4_9_1a_{STAMP}"
REPORT = OUT_DIR / f"bao_cao_kiem_thu_4_9_1a_{STAMP}.txt"
...
def choose_year(db):
    ...
    requested = str(os.environ.get("PHOCAP_TEST_YEAR", "") or "").strip()

    if requested:
        year = db.scalar(
            select(SchoolYear).where(SchoolYear.code == requested)
        )
        if year is None:
            raise RuntimeError(
                f"PHOCAP_TEST_YEAR={requested} nhưng database không có năm học này."
            )
        return year
...
def run() -> int:
    ...
    try:
        ...
        try:
            ...
            school_year = choose_year(db)
            print(
                f"[3/7] Năm học kiểm thử: "
                f"{school_year.code} (ID={school_year.id})."
            )

            special_info, normal_info = choose_communes(db, school_year)
            ...
            rows.extend(
                [
                    "=" * 104,
                    "BÀI 13B-11.15.2.4.9.1A - KẾT QUẢ KIỂM THỬ THỰC TẾ",
                    "=" * 104,
                    "",
        ...
        rows.extend(
            [
                ...
                "",
                "KẾT LUẬN: BÀI 4.9.1A ĐẠT KIỂM THỬ KỸ THUẬT THỰC TẾ.",
            ]
        )
        ...
        print()
        print("=" * 104)
        print("BÀI 4.9.1A - KIỂM THỬ THỰC TẾ HOÀN THÀNH.")
        ...
        print()
        print("=" * 104)
        print("[LỖI] BÀI 4.9.1A CHƯA ĐẠT")
        print(str(exc))
        print("=" * 104)

        try:
            OUT_DIR.mkdir(parents=True, exist_ok=True)
            error_report = OUT_DIR / f"loi_kiem_thu_4_9_1a_{STAMP}.txt"
