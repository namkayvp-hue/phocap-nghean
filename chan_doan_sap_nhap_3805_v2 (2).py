...
def index_rows(conn: sqlite3.Connection, table: str) -> list[dict]:
    """
    Doc danh sach index theo cach an toan.

    SQLite co the tra ve None o cot name cua PRAGMA index_info()
    doi voi expression index (cid = -2). Ban V1 dua None vao join()
    nen phat sinh TypeError. Ban V2 chuyen cac thanh phan dac biet
    thanh chuoi <expression>, dong thoi van giu ten cot that de doi
    chieu school_id.
    """
    result: list[dict] = []
    ...
    for idx in indexes:
        ...
        idx_name_raw = idx[1] if len(idx) > 1 else None
        ...
        result.append({
            ...
            "origin": str(idx[3] or "") if len(idx) > 3 else "",
            "columns": cols,
        })
...
def main() -> int:
    print("CHAN DOAN SAP NHAP TRUONG THEO QD 3805/QD-UBND - V2")
    ...
    try:
        ...
        with summary.open("w", encoding="utf-8") as f:
            f.write("CHAN DOAN SAP NHAP TRUONG THEO QD 3805/QD-UBND - V2\n")
