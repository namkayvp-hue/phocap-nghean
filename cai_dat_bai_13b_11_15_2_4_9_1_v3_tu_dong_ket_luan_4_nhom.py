...
def top_level_symbol_exists(source: str, name: str) -> bool:
    """
    Kiểm tra một tên có sẵn ở module qua:
    - def / async def / class
    - import ... as ...
    - from ... import ...
    - phép gán top-level

    Dùng cho helper được import như _percent của builder Mầm non.
    """
    tree = ast.parse(source)
...
def patch_mn_source(source: str) -> str:
    ...
    if not top_level_symbol_exists(source, "_percent"):
        raise RuntimeError(
            "Builder Mầm non không có symbol _percent "
            "(định nghĩa hoặc import)."
        )
...
def build_report(before: dict, after: dict) -> str:
    lines = [
        "=" * 96,
        "BÀI 13B-11.15.2.4.9.1 V3 - BÁO CÁO CÀI ĐẶT",
        "=" * 96,
        "",
        "PHẠM VI:",
