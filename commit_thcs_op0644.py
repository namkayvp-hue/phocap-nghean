...
LOG_PATH = EXPORT_DIR / "COMMIT_THCS_OP0644.txt"

# Nền chính xác sau COMMIT OP-0210 V2.
EXPECTED_DB_SHA = "10556d7c21295e19a83d4c31936672e3aca36569b405e96247d6aac6cafc529a"

PLAN_ID = "PA2026-52415E680B77"
OFFICIAL_OPERATION_CODE = "QD3805-OP-0644"
PREVIEW_FINGERPRINT = "2668d7fcfce6712e57f67d53490d8e180d719216c2f524ac591bce9d64ebb26e"
...
def validate_plan(plan):
    ...
    if plan.get("commune_excel") != "Yên Hòa":
        fail("Plan sai commune_excel")
    if plan.get("plan_text") != "PTDTBT THCS Yên Thắng":
    ...
    if len(members) != 2:
        fail(f"Plan phải đúng 2 member, hiện có {len(members)}")
    if not any("Yên Thắng" in n for n in names):
        fail("Plan thiếu PTDTBT THCS Yên Thắng")
    if not any("Yên Hòa" in n for n in names):
        fail("Plan thiếu PT DTBT Yên Hòa")

    log("ALIAS_POLICY = 'PT DTBT Yên Hòa' được khóa vào SOURCE school_id=1748, code=40418519.")
...
def unrelated_snapshot(conn):
    """
    Chỉ cho phép:
      - đổi schools.id=1748 -> is_active=0;
      - đổi tài khoản cấp trường của 1748 -> is_active=0;
      - thêm staff_year_records target=1747, year=2;
      - thêm 1 operation + 1 official execution.
    Mọi dữ liệu khác phải bất biến.
    """
    snap = {}
...
def log_table_baseline(conn, table):
    ...
    return rows_hash(cur), conn.execute(
...
def make_backup():
    ...
    backup_path = BACKUP_DIR / f"backup_truoc_THCS_OP0644_{ts}.db"
...
def clone_staff(conn, base_rows):
    insert_columns = [r[1] for r in table_info(conn, "staff_year_records") if r[1] != "id"]
    sql = (
...
def main():
    ...
    log("="*150)
    log("COMMIT QD3805-OP-0644")
    log("THCS Yên Hòa (1748) -> PTDTBT THCS Yên Thắng (1747)")
    log("="*150)
    ...
    if sha_before != EXPECTED_DB_SHA:
        fail("DB SHA đã khác nền sau OP-0210. Không COMMIT.")
    ...
    try:
        ...
        summary_obj = {
            "special_execution_type": "THCS_FULL_SOURCE_MERGE_ALIAS_LOCKED",
            ...
            "source_alias_in_qd3805": "PT DTBT Yên Hòa",
        ...
        if diffs:
            fail("Phát hiện dữ liệu ngoài write-set bị thay đổi - ROLLBACK")
