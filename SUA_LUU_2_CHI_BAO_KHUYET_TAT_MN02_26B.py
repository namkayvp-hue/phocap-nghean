...
OUT = EXPORTS / "SUA_LUU_2_CHI_BAO_KHUYET_TAT_MN02_26B.txt"

EXPECTED_DB_SHA = "cb821b5500b239756ff4b745b8f36abfaa8a44b9f43de00c26a64cd9db81ce26"

FIX_START = "# === FIX26B_SAVE_DISABILITY_REPORT_FLAGS_START ==="
FIX_END = "# === FIX26B_SAVE_DISABILITY_REPORT_FLAGS_END ==="
...
def remove_fix26_blocks(fn: str) -> str:
    # Xóa block FIX26/FIX26B cũ nếu có để cài lại idempotent.
    patterns = [
        re.compile(
...
def insert_before_commit(fn: str) -> tuple[str, int]:
    ...
    i = m.group("i")
    block = (
        f"{i}# === FIX26B_FINAL_ASSIGN_BEFORE_COMMIT ===\n"
...
def main():
    ...
    with OUT.open("w", encoding="utf-8-sig", newline="\n") as f:
        log(f, "=" * 150)
        log(f, "FIX 26B - SỬA LƯU 2 CHỈ BÁO KHUYẾT TẬT PHỤC VỤ MN-02")
        log(f, "BẢN SỬA SAU LỖI 26A: VERIFIER KHÔNG CÒN ĐẾM NHẦM record BÊN TRONG form_record")
        ...
        backup_root = BACKUPS / f"source_truoc_FIX26B_{stamp}"
