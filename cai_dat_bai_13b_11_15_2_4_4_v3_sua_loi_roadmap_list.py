...
# === BAI_13B_11_15_2_4_4_V2_FIX_JSON_BOOLEAN ===
# === BAI_13B_11_15_2_4_4_V3_FIX_ROADMAP_LIST ===
# Hỗ trợ ROADMAP_RECORDS là list Python hoặc chuỗi JSON.
# Sửa lỗi boolean JSON khi nhúng lộ trình vào Python.
...
def main() -> None:
    ...
    # V3: ROADMAP_RECORDS có thể là list Python hoặc chuỗi JSON.
    # Không gọi json.loads() trực tiếp lên list.
    if isinstance(ROADMAP_RECORDS, str):
        records = json.loads(ROADMAP_RECORDS)
    elif isinstance(ROADMAP_RECORDS, list):
        records = [dict(item) for item in ROADMAP_RECORDS]
    else:
        raise TypeError(
            "ROADMAP_RECORDS phải là list hoặc chuỗi JSON; "
            f"nhận được {type(ROADMAP_RECORDS).__name__}"
        )
