...
BACKUP = (
    PROJECT
    / "exports"
    / f"backup_bai_13b_11_15_2_3_2_{STAMP}"
)
...
def verify_template(
    ...
    if re.search(
        r"\bMutationObserver\s*\(",
        new_script[2],
    ):
        raise RuntimeError(
            "Bài 15.2.3 không được gọi MutationObserver(...)."
        )
...
def main() -> int:
    ...
    # Kiểm tra regex trước mọi thao tác file/database.
    _mutation_pattern = re.compile(
        r"\bMutationObserver\s*\("
    )
    print("SỬA SO VỚI V15.2.3.1:")
    print(
        " - V15.2.3.1 escape dư dấu \\ trong regex kiểm tra."
    )
    print(
        " - V15.2.3.2 dùng regex hợp lệ: "
        r"\bMutationObserver\s*\("
    )
    print(
        " - Không thay đổi logic ẩn/hiện theo cấp học."
    )
    print()
    print("LỊCH SỬ V15.2.3:")
    print(
        " - V15.2.3 bắt nhầm chữ MutationObserver trong comment."
    )
    print(
        " - V15.2.3.1 chỉ chặn lời gọi MutationObserver(...) thực sự."
    )
    print(
        " - Không thay đổi logic ẩn/hiện theo cấp học."
    )
    ...
    if re.search(
        ...
        raise RuntimeError(
            "Script hiện tại vẫn gọi MutationObserver(...). "
    ...
    try:
        ...
        print("=" * 132)
        print(
            "CÀI ĐẶT BÀI 13B-11.15.2.3.2 THÀNH CÔNG"
        )
        print("=" * 132)
