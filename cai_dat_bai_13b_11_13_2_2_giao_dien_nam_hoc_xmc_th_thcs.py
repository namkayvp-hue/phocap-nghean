...
BACKUP = PROJECT / "exports" / f"backup_bai_13b_11_13_2_2_{STAMP}"
...
def _is_backup_router_file(path: Path) -> bool:
    ...
    return any(token in stem for token in backup_tokens)


def find_save_route() -> tuple[Path, ast.FunctionDef | ast.AsyncFunctionDef]:
    # Bài 13B-11.13.2.1:
    # Route nghiệp vụ thật hiện nằm trong app/routers/surveys.py.
    # Ưu tiên tuyệt đối file chính để không nhầm với các file source backup
    # còn nằm trong app/routers.
    canonical = ROUTERS / "surveys.py"

    canonical_matches = _find_save_routes_in_file(
        canonical
    )
    ...
    if len(canonical_matches) > 1:
        detail = "\n".join(
            ...
            for p, n in canonical_matches
        )
        raise RuntimeError(
            "Trong app/routers/surveys.py có nhiều route POST /nam-hoc/luu; "
            "dừng để tránh sửa nhầm.\n"
            + detail
        )

    # Fallback chỉ dùng nếu cấu trúc dự án đã thay đổi.
    matches = []
    ...
    for path in ROUTERS.rglob("*.py"):
        ...
        if _is_backup_router_file(path):
            continue

        matches.extend(
            _find_save_routes_in_file(path)
        )
    ...
    if len(matches) != 1:
        ...
        raise RuntimeError(
            "Không xác định được duy nhất route nghiệp vụ POST /nam-hoc/luu "
            "sau khi đã bỏ qua source backup. "
...
def patch_router(
    ...
    # Bài 13B-11.13.2.2:
    # Không dùng str.format() cho toàn khối vì trong code có
    # các set literal như {"CO", "1", "TRUE"}.
    # Chỉ thay đúng placeholder {record}.
    save_block = SAVE_BLOCK_TEMPLATE.replace(
        "{record}",
        record_name,
    )
...
def main() -> int:
    print("=" * 124)
    print(
        "BÀI 13B-11.13.2.2 - GIAO DIỆN THÔNG TIN NĂM HỌC ĐỘNG "
    ...
    print(
        " - Ưu tiên app/routers/surveys.py; bỏ qua source backup cũ."
    )
    print(
        " - Tự kiểm tra route POST có /nam-hoc/luu bằng AST."
    )
    print(
        " - Bản 2.2 sửa an toàn lỗi template Python có dấu ngoặc nhọn."
    )
    print(
    ...
    try:
        ...
        print()
        print(
            "CÀI ĐẶT BÀI 13B-11.13.2.2 THÀNH CÔNG"
        )
