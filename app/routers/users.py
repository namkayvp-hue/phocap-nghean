...
def chuan_hoa_cap_tai_khoan(value: str | None) -> str | None:
    code = str(value or "").strip().upper()
    return code if code in SCHOOL_ACCOUNT_LEVEL_LABELS else None
...
def lay_danh_sach_don_vi_phan_trang(
    *,
    ...
    if section == "xa":
        ...
        include_archived_units = bool(keyword.strip()) or account_status == "locked" or school_id is not None
        unit_statement = (
            select(School)
            .join(School.commune)
        ...
        if not include_archived_units:
            unit_statement = unit_statement.where(School.is_active.is_(True))
        ...
        if school_level is not None:
            level_school_ids = (
                ...
                .where(SchoolNetworkYearData.level_code == school_level)
                .distinct()
            )
            unit_statement = unit_statement.where(School.id.in_(level_school_ids))
...
def lay_danh_sach_tai_khoan_phan_trang(
    *,
    ...
    # === BAI_13B_11_11_2_6_TEACHER_NAME_ONLY_START ===
...
def tao_url_bo_loc(
    *,
    ...
    if section == "truong" and school_level is not None:
        params["cap"] = school_level
...
def danh_sach_tai_khoan(
    ...
    school_level = chuan_hoa_cap_tai_khoan(cap) if section == "truong" else None
    ...
    if actor_role in ADMIN_ROLE_CODES:
        include_archived_school_options = bool(keyword) or account_status == "locked" or school_id is not None
        school_statement = (
            select(School)
            .join(School.commune)
            .where(Commune.is_active.is_(True))
        ...
        if not include_archived_school_options:
            school_statement = school_statement.where(School.is_active.is_(True))
        if commune_id is not None:
        ...
        if school_level is not None:
            ...
            school_statement = school_statement.where(School.id.in_(level_school_ids))
...
school_level=school_level,
    ...
    if section == "truong" and school_level is not None:
        section_title = f"Tài khoản Trường {SCHOOL_ACCOUNT_LEVEL_LABELS[school_level]}"
    ...
    return templates.TemplateResponse(
        ...
        context={
            ...
            "tieu_de_muc": section_title,
            "cap_hoc": school_level,
            "cap_hoc_label": SCHOOL_ACCOUNT_LEVEL_LABELS.get(school_level),
            "tong_hop": summary,
            "tu_khoa": keyword,
...
@router.get("/cap-tat-ca-theo-cap", response_class=HTMLResponse)
def xem_truoc_cap_tat_ca_tai_khoan_theo_cap(
    request: Request,
    db: Session = Depends(get_db),
    cap: str = Query(default=""),
    status: str | None = None,
    ...
    school_level = chuan_hoa_cap_tai_khoan(cap)
    if school_level is None:
        return RedirectResponse("/tai-khoan?muc=truong", 303)

    all_rows, _ = lay_danh_sach_don_vi_phan_trang(
        db=db,
        section="truong",
        keyword="",
        commune_id=None,
    ...
    candidates, _ = lay_danh_sach_don_vi_phan_trang(
        db=db,
        section="truong",
        keyword="",
        commune_id=None,
    ...
    level_label = SCHOOL_ACCOUNT_LEVEL_LABELS[school_level]
    return templates.TemplateResponse(
        request=request,
        name="users/bulk_level_all_preview.html",
        context={
            "nguoi_dung": actor,
            "cap_hoc": school_level,
            "cap_hoc_label": level_label,
            "tong_truong": len(all_rows),
            ...
            "url_quay_lai": f"/tai-khoan?muc=truong&cap={school_level}",
            "thong_bao": STATUS_MESSAGES.get(status),
            "trang_thai": status,
        },
    )
...
def cap_tat_ca_tai_khoan_theo_cap(
    request: Request,
    ...
    preview_url = f"/tai-khoan/cap-tat-ca-theo-cap?cap={school_level}"
    ...
    return_url = f"/tai-khoan?muc=truong&cap={school_level}"
    scope_label = f"Toàn bộ trường {level_label} chưa có tài khoản"
    filters_data = {
        "school_level": school_level,
        "scope_label": scope_label,
    ...
    batch_result = db.execute(
        text(
            ...
            INSERT INTO account_provision_batches (
                ...
                'truong', :scope_type, :filters_json, :total_candidates,
        ...
        db.execute(
            text(
                ...
                INSERT INTO account_provision_batch_items (
                    ...
                    :batch_id, 'truong', :unit_id, :commune_id, :school_id,
            ...
            ),
            {
                ...
                "unit_code": unit.code,
                "unit_name": unit.name,
                "result_note": f"Chờ cấp tài khoản trường {level_label}",
                "created_at": created_at,
            },
        )
