    ...
    for row in raw_rows:
        ...
        if school_display:
            ...
            school_item["completed_preschool_5"] += (
                1 if _v243_completed_preschool(row) else 0
            )
        ...
        summary["completed_preschool_5"] += 1 if _v243_completed_preschool(row) else 0
        ...
        commune_item["completed_preschool_5"] += 1 if _v243_completed_preschool(row) else 0
...
def _append_detail_sheet(workbook: Workbook, data: dict[str, Any]) -> None:
    ...
    for index, row in enumerate(data["person_rows"], start=1):
        sheet.append([
            ...
            row["class_display"], "Có" if _v243_completed_preschool(row) else "Không" if row.get("completed_preschool_5") is False else "Chưa xác định",
