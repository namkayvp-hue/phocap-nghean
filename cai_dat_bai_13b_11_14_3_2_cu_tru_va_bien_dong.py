...
BACKUP = PROJECT / "exports" / f"backup_bai_13b_11_14_3_2_{STAMP}"
...
def find_year_get_function(source: str) -> str:
    ...
    # Bài 13B-11.14.3.1:
    # Route POST lưu cũng có TemplateResponse để trả lại form khi lỗi,
    # nên không được nhận diện chỉ bằng nội dung hàm.
    # Ưu tiên đúng tên route chuẩn của dự án.
    canonical_name = "theo_doi_nam_hoc_doi_tuong"
    ...
    def _is_get_decorator(node) -> bool:
    ...
    if len(matches) != 1:
        raise RuntimeError(
            "Phải tìm thấy đúng 1 route GET thật sự "
...
def patch_get_route(source: str):
    ...
    # Bài 13B-11.14.3.2:
    # Không giả định context phải có đúng:
    # "selected_school_year_id": selected_school_year_id
    #
    # Ta tìm chính xác TemplateResponse của year_records.html.
    # Nếu context là dict literal -> chèn key trực tiếp vào dict.
    # Nếu context là biến -> gán context["b131143_events"] trước return.
    ...
    if function_node is None:
        raise RuntimeError(
            "Không đọc được AST route GET năm học."
        )
    ...
    for node in ast.walk(function_node):
        ...
        call_source = (
            ast.get_source_segment(fn, call)
            or ""
        )

        if "year_records.html" not in call_source:
            continue
    ...
    if template_call is None or return_node is None:
        raise RuntimeError(
            "Không tìm thấy TemplateResponse "
            "của year_records.html."
        )

    # ------------------------------------------------------------
    # 1. Chèn lệnh đọc sự kiện ngay trước đúng return year_records.
    # ------------------------------------------------------------
    if GET_MARKER not in fn:
        lines = fn.splitlines(keepends=True)

        insert_index = int(return_node.lineno) - 1
        return_line = lines[insert_index]
        indent = (
            return_line[
                : len(return_line)
                - len(return_line.lstrip())
            ]
        )
        ...
        load_block = (
            ...
            f"{indent}    db,\n"
            f"{indent}    person.id,\n"
            f"{indent}    selected_school_year_id,\n"
            f"{indent})\n"
            f"{indent}"
    ...
    # Sau khi chèn block, parse lại để line number/context đúng.
    tree = ast.parse(fn)
    function_node = next(
        node
        for node in tree.body
    ...
    for node in ast.walk(function_node):
        ...
        if not (
            ...
            and call.func.attr == "TemplateResponse"
        ):
            continue
        ...
        if "year_records.html" in call_source:
            template_call = call
            return_node = node
            break
    ...
    if template_call is None or return_node is None:
        raise RuntimeError(
            "Không tìm lại được TemplateResponse "
            "sau khi chèn block sự kiện."
        )

    if '"b131143_events"' in (
    ...
    # FastAPI/Jinja phổ biến:
    # templates.TemplateResponse("...", {...})
    # hoặc TemplateResponse(name="...", context={...})
    context_node = None

    if len(template_call.args) >= 2:
        context_node = template_call.args[1]

    for keyword in template_call.keywords:
        if keyword.arg == "context":
            context_node = keyword.value
            break

    if context_node is None:
        raise RuntimeError(
            "Không xác định được context "
            "của TemplateResponse year_records.html."
        )

    lines = fn.splitlines(keepends=True)

    if isinstance(context_node, ast.Dict):
        # Chèn vào ngay sau dấu { mở đầu của dict context.
        start_line = int(context_node.lineno) - 1
        end_line = int(context_node.end_lineno) - 1

        # Tìm dòng đầu trong range có dấu { của context.
        inserted = False
        ...
        for line_index in range(
            ...
            # Các key trong context thường thụt thêm 4 spaces.
        ...
        if not inserted:
            raise RuntimeError(
                "Không chèn được b131143_events "
                "vào dict context."
            )

        fn = "".join(lines)

    elif isinstance(context_node, ast.Name):
        context_name = context_node.id

        return_line_index = (
            int(return_node.lineno) - 1
        )
        ...
        raise RuntimeError(
            "Kiểu context TemplateResponse "
            "chưa hỗ trợ an toàn: "
            + type(context_node).__name__
            + " :: "
            + context_source[:120]
        )
    ...
    if '"b131143_events"' not in fn:
        raise RuntimeError(
            "Sau patch vẫn chưa có "
            "b131143_events trong context."
        )
...
def main() -> int:
    ...
    print(
        " - V14.3.2 nhận diện đúng route GET và chèn dữ liệu "
        "vào chính context TemplateResponse year_records.html."
    )
    print()
    ...
    try:
        ...
        print("=" * 124)
        print(
            "CÀI ĐẶT BÀI 13B-11.14.3.2 THÀNH CÔNG"
        )
        print("=" * 124)
