# -*- coding: utf-8 -*-
"""
BÀI 13B-11.15.2.4.9.1A.2
KIỂM THỬ THỰC TẾ - QUÉT TOÀN BỘ 51 XÃ ĐBKK
...
OUT_DIR = EXPORTS / f"kiem_thu_bai_13b_11_15_2_4_9_1a_2_{STAMP}"
REPORT = OUT_DIR / f"bao_cao_kiem_thu_4_9_1a_2_{STAMP}.txt"
...
def choose_year_with_real_data(db):
    """
    Quét toàn bộ 130 xã/phường theo từng năm học.

    Mỗi năm:
    - tìm xã ĐBKK có nhiều bản ghi năm học nhất;
    - tìm xã thường có nhiều bản ghi năm học nhất.

    Chọn năm MỚI NHẤT mà cả 2 nhóm đều có dữ liệu > 0.
    Nếu không có năm nào đạt, trả None cùng bảng scan để báo cáo.
    """
    from sqlalchemy import select
    ...
    specials = list(
        db.scalars(
            ...
            .where(
                ...
                Commune.is_special_difficulty_area == 1,
            )
    ...
    if len(specials) != EXPECTED_SPECIAL_COMMUNES:
        raise RuntimeError(
            f"Nhóm ĐBKK có {len(specials)} xã/phường, "
    ...
    for year in years:
        best_special = None
        best_special_count = -1

        for commune in specials:
            count = commune_data_count(
                db,
                int(commune.id),
                int(year.id),
            )
            if count > best_special_count:
                best_special = commune
                best_special_count = count
        ...
        scan.append(
            {
                ...
                "special_count": int(max(best_special_count, 0)),
                "normal": best_normal,
...
def choose_communes(db, school_year):
    """
    Hàm tương thích; tự chọn xã ĐBKK và xã thường nhiều dữ liệu nhất
    trong năm đã cho.
    """
    from sqlalchemy import select
    from app.models import Commune
    ...
    def best(items):
        ...
        for item in items:
            ...
            scored.append((int(count), str(item.name), item))
    ...
    special, special_count = best(specials)
    normal, normal_count = best(normals)

    if special is None:
        raise RuntimeError("Không có xã ĐBKK.")
    if normal is None:
        raise RuntimeError("Không có xã/phường bình thường.")
    ...
    try:
        ...
        try:
            ...
            for item in year_scan:
                ...
                print(
                    ...
                    f"ĐBKK tốt nhất={special_name}: "
                    f"{item['special_count']} | "
            ...
            if chosen is None:
                raise RuntimeError(
                    "Không có năm học nào có dữ liệu > 0 đồng thời "
                    "ở ít nhất 1 trong 51 xã ĐBKK và ít nhất 1 xã/phường thường. "
                    "Database hiện chưa đủ dữ liệu thực để kiểm thử hai nhánh ngưỡng."
                )
            ...
            special_info = {
                "kind": "DBKK",
                "commune": chosen["special"],
                "data_count": chosen["special_count"],
            }
            normal_info = {
                "kind": "BINH_THUONG",
            ...
            rows.extend(
                [
                    "=" * 104,
                    "BÀI 13B-11.15.2.4.9.1A.2 - KẾT QUẢ KIỂM THỬ THỰC TẾ",
                    "=" * 104,
                    "",
                    f"Năm học: {school_year.code} (ID={school_year.id})",
                    f"Database: {DB_PATH}",
                    f"Địa bàn ĐBKK được chọn: {special_info['commune'].name}",
        ...
        rows.extend(
            [
                ...
                "",
                "KẾT LUẬN: BÀI 4.9.1A.2 ĐẠT KIỂM THỬ THỰC TẾ HAI NHÓM ĐỊA BÀN.",
            ]
        )
        ...
        print()
        print("=" * 104)
        print("BÀI 4.9.1A.2 - KIỂM THỬ THỰC TẾ HOÀN THÀNH.")
        ...
        print()
        print("=" * 104)
        print("[LỖI] BÀI 4.9.1A.2 CHƯA ĐẠT")
        print(str(exc))
        print("=" * 104)

        try:
            OUT_DIR.mkdir(parents=True, exist_ok=True)
            error_report = OUT_DIR / f"loi_kiem_thu_4_9_1a_2_{STAMP}.txt"
